...
def code_reward_logger(
    completions1, completions2, test_cases, entry_points, prompts=None, task_descriptions=None
):
    """
    ...
    Args:
        ...
        task_descriptions: Optional list of task descriptions (instruct_prompt for BigCodeBench)
    ...
    # Handle case where task_descriptions is not provided
    if task_descriptions is None:
        task_descriptions = [""] * len(completions1)

    for i, (c1, c2, test_code, entry_point, prompt, task_desc) in enumerate(
        zip(completions1, completions2, test_cases, entry_points, prompts, task_descriptions)
    ):
        
        metrics = {
            "sample_id": i,
            "entry_point": entry_point,
            "task_description": task_desc,  # NEW: Task description
            # Level 1 metrics
            "level_1_reward": 0.0,
            # Level 2 metrics
            "level_2_reward": 0.0,
...
def code_reward_logger_bigcodebench(
    completions1, completions2, test_cases, entry_points, code_prompts=None, task_descriptions=None
):
    """
    Logger for BigCodeBench code generation tasks with aux + main function collaboration.
    Uses unittest-based test execution (same as execution_reward_bigcodebench).

    Args:
        completions1: List of aux function completions
        completions2: List of main function completions
        test_cases: List of test cases (unittest format)
        entry_points: List of entry point function names
        code_prompts: Optional list of code_prompts for import extraction
        task_descriptions: Optional list of task descriptions (instruct_prompt)
    ...
    all_metrics = []
    TEST_TIMEOUT = 5  # Same as execution_reward_bigcodebench

    # Handle case where code_prompts is not provided
    if code_prompts is None:
        code_prompts = [""] * len(completions1)
    
    # Handle case where task_descriptions is not provided
    if task_descriptions is None:
        task_descriptions = [""] * len(completions1)

    for i, (c1, c2, test_code, entry_point, code_prompt, task_desc) in enumerate(
        zip(completions1, completions2, test_cases, entry_points, code_prompts, task_descriptions)
    ):
        # Truncate task description for logging (first 200 chars)
        ...
        # Extract imports from code_prompt
        ...
        # Calculate test metrics
        if total_tests > 0:
            metrics["passed_rate"] = passed_tests / total_tests
            metrics["test_reward"] = metrics["passed_rate"] * 1.0
            # Check if all tests passed
