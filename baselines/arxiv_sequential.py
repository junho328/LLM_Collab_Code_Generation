...
class SequentialTwoAgentQwenarXivBaseline:
    def __init__(self, model_name="Qwen/Qwen3-1.7B", device="auto"):
        """
        Initialize two Qwen models for sequential arXiv baseline evaluation.
    ...
    def background_agent_formatter(self, example: Dict[str, Any]) -> str:
        """
        Formatter for the background agent (Agent 1).
        Takes the abstract text and expand its background information.
        """
        ...
        prompt_text = f"""Create an expanded introduction background to this abstract.

Query:
{prompt}

IMPORTANT INSTRUCTIONS:
- Provide an expanded background introduction to the abstract.
- Don't need to talk about the methodology or implications.
...
Summary:"""
    ...
    def complementary_agent_formatter(
        ...
        target_length = max(
            agent1_word_count * 1.2, 80
        )  # At least 2x longer, minimum 80 words

        prompt_text = f"""You are Agent 2 in a two-agent system. Your job is to create introduction based on Agent 1's initial response.
...
Your task is to create a complementary introduction that:
1. Is approximately {target_length} words long (about 1.2 times longer than Agent 1's response)
2. Uses similar number of unique vocabulary words than Agent 1
3. Includes transition words to improve flow
4. Don't talk about the background and motivation of the abstract.
...
Improved Summary:"""
    ...
    def evaluate_arxiv_baseline(
        ...
        """
        Evaluate arXiv baseline with sequential two agents.
        ...
        agent1_completions = []  # Agent 1 completions
        agent2_completions = []  # Agent 2 completions
        response_times = []
        failed_samples = 0
        ...
        for i, sample in enumerate(
            ...
            try:
                ...
                # STEP 1: Generate Agent 1 response
                agent1_prompt_text = self.background_agent_formatter(sample)
                ...
                agent2_prompt_text = self.complementary_agent_formatter(
        ...
        if save_results:
            results = {
                ...
                "agent1_role": "initial_background_agent",
                "agent2_role": "complementary_agent_with_agent1_context",
            ...
            filename = (
                f"arxiv_sequential_two_agent_baseline_results_{int(time.time())}.json"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="arXiv Sequential Two-Agent Baseline Evaluation"
    )
    ...
    evaluator = SequentialTwoAgentQwenarXivBaseline(
